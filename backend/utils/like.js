...
async function likeUser(userId, message) {
  ...
  userliked = await pool.query("SELECT * FROM users WHERE LOWER(username) = LOWER($1)", [
    userLikedM,
  ]);
  ...
  if (
    userBlacklist.includes(userliked.id.toString()) ||
    userlikedBlacklist.includes(user.id.toString())
  ) {
    ws.send(
      JSON.stringify({
        type: "error",
  ...
  await pool.query(
    ...
    [{ title: "like", body: user.username + " liked you" }, userliked.id]
  );
  await pool.query("UPDATE users SET famerating = $1 WHERE id = $2", [
    userliked.famerating,
    userliked.id,
  ]);
  ws.send(
    JSON.stringify({
  ...
  if (matcha) {
    await pool.query("UPDATE users SET matcha = array_append(matcha, $1) WHERE id = $2", [
      userliked.id,
      user.id,
    ]);
    await pool.query("UPDATE users SET matcha = array_append(matcha, $1) WHERE id = $2", [
    ...
    await pool.query(
      "UPDATE users SET notifications = array_append(notifications, $1) WHERE id = $2",
      [{ title: "match", body: userliked.username + " matched with you" }, user.id]
    );
    await pool.query(
      "UPDATE users SET notifications = array_append(notifications, $1) WHERE id = $2",
      [{ title: "match", body: user.username + " matched with you" }, userliked.id]
    );
    user.famerating += 50;
