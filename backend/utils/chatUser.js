...
async function chatUser(userId, message) {
  ...
  try {
    ...
    const userRecipientResult = await pool.query(
    ...
    if (
      userBlacklist.includes(userRecipient.id.toString()) ||
      recipientBlacklist.includes(user.id.toString())
    ) {
      return ws.send(
        JSON.stringify({
          type: "error",
    ...
    if (
      !userMatcha.includes(userRecipient.id.toString()) &&
      !recipientMatcha.includes(user.id.toString())
    ) {
      return ws.send(
        JSON.stringify({
          type: "error",
